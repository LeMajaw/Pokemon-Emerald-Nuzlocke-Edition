...
// Day/night acceleration: one full 24h in-game cycle == this many gameplay
// frames (VBlanks) at normal speed. 60 min * 60 s * 60 fps = 216000, i.e. one
// cycle per 60 minutes of play. Frame-based, so it speeds up under emulator
// fast-forward (unlike the real RTC, which always ticks at wall-clock seconds).
#define DAY_NIGHT_CYCLE_GAMEPLAY_FRAMES 216000

// Returns the accelerated in-game hour (0..23) and, if minutesOut is non-NULL,
// the in-game minute (0..59), derived from active GAMEPLAY time - the play-time
// counter in SaveBlock2 (which increments every frame in the overworld, battles
// and menus; see PlayTimeCounter_Update) - NOT the real RTC. Shared by
// UpdateTimeOfDay (visuals) and GetDayOrNight (encounters) so the two can never
// drift apart, and saved automatically via the play-time fields. Because one
// cycle == 60 play-minutes (one play-hour), the position within the cycle is
// just the play minute+second+vblank of the current hour, so there is no
// overflow and no need to mod a large running frame total. The real RTC is
// untouched, so berries, tides, lottery, Mirage Island and daily events keep
// real-world timing.
s32 GetAcceleratedTimeOfDay(s32 *minutesOut)
{
    // Frame position within the current 60-play-minute cycle (0..215999).
    s32 cycleFrame = (gSaveBlock2Ptr->playTimeMinutes * 60 + gSaveBlock2Ptr->playTimeSeconds) * 60
                   + gSaveBlock2Ptr->playTimeVBlanks;
    s32 inGameMin = cycleFrame * 1440 / DAY_NIGHT_CYCLE_GAMEPLAY_FRAMES; // 0..1439
    if (minutesOut != NULL)
